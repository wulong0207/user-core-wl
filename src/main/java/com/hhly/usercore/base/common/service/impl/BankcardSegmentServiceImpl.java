...
public class BankcardSegmentServiceImpl implements BankcardSegmentService {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<PayBankSegmentBO> selectGroup() {
        String key = CacheConstants.PAY_BANK_SEGMENTBO_LIST_KEY;
        Object object = redisUtil.getObj(key);
        List<PayBankSegmentBO> list;
        if (ObjectUtil.isBlank(object)) {
            list = payBankSegmentMapper.selectGroup();
            // 设置缓存，永不过期
            redisUtil.addObj(key, list, CacheConstants.TWELVE_HOURS);
        } else {
            list = (List<PayBankSegmentBO>) object;
        }
        return list;
    ...
    public ResultBO<?> findPayBankSegmentByCard(String bankCard) {
        ...
        List<PayBankSegmentBO> payBankSegmentBOList = findList();
        ...
        for (PayBankSegmentBO payBankSegmentBO : payBankSegmentBOList) {
            if (!ObjectUtil.isBlank(payBankSegmentBO.getTopCut())) {
                // 先检索是不是符合xxxxx开头的
                if (bankCard.startsWith(String.valueOf(payBankSegmentBO.getTopCut())) && !ObjectUtil.isBlank(payBankSegmentBO.getBankId())) {
                    bo = payBankSegmentBO;
                    break;
                }
            }
